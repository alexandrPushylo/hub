...
def get_bills(bill: type[Electricity] | type[Water] | type[Rent]) -> dict:
    out = {}
...
def get_info_bills(bill: type[Electricity] | type[Water] | type[Rent]) -> dict:
    if bill in (Electricity, Water):
...
def set_bills(bill: type[Electricity] | type[Water] | type[Rent], data: dict) -> bool:
    try:
        ...
        if new_bill:
            ...
            if bill not in (Rent,):
                ...
                new_bill.calculate_amount(diff_indications)
            else:
